...
/** Number of shards for chunk map â must be power of two. */
...
/** Per-shard mutexes protecting concurrent inserts/erases. */
static std::shared_mutex chunk_map_mutexes[CHUNK_MAP_SHARDS];
...
/** Registers a chunk to the sharded buf_pool_chunk_map.
Does NOT register rw-locks â they are initialized lazily.
...
// Use frame address as shard key.
  ...
  std::unique_lock<std::shared_mutex> lock(chunk_map_mutexes[shard_idx]);
...
Uses an atomic CAS to guarantee thread safety without a global lock.
@param[in,out]  block   buffer block */
inline void buf_block_ensure_latches_initialized(buf_block_t *block) {
...
// Another thread is initializing. Wait with exponential backoff.
    uint32_t spins = 0;
    while (!block->latches_initialized.load(std::memory_order_acquire)) {
      if (spins < 32) {
        ut_delay(1);
        ++spins;
        }
...
// This thread won the CAS â initialize the latches.
...
/** Lightweight initialization of a buffer control block â no latches created.
...
// Latches are NOT initialized here â done lazily on first use.
...
// Unregister already-registered chunks before freeing them.
        while (--chunk >= buf_pool->chunks) {
          const byte *frame = chunk->blocks->frame;
          size_t shard_idx =
            reinterpret_cast<uintptr_t>(frame) % CHUNK_MAP_SHARDS;
            {
            std::unique_lock<std::shared_mutex> lock(chunk_map_mutexes[shard_idx]);
            if (buf_chunk_map_shards[shard_idx] != nullptr) {
              buf_chunk_map_shards[shard_idx]->erase(frame);
              }
            }
...
when doing a fast shutdown. */
  ...
  std::vector<const byte *> frame_addrs;
...
// Unregister chunks from sharded map.
  for (const auto *frame_addr : frame_addrs) {
    size_t shard_idx =
...
dberr_t buf_pool_init(ulint total_size, bool populate, ulint n_instances) {
  ...
  buf_pool_ptr = static_cast<buf_pool_t *>(
...
/* CPU-based limit for parallel init threads.
Hard limit is the number of buffer pool instances. */
  if (srv_buffer_pool_parallel_init_threads > 0) {
    n_cores = std::min<ulint>(srv_buffer_pool_parallel_init_threads,
      n_instances);
    }
  else {
    if (n_cores > 8) {
...
#else
  ulint n_cores = std::min<ulint>(4, n_instances);
#endif                                          /* UNIV_LINUX */
  ...
  double speed_gb_per_sec = 0.0;
  const double total_size_bytes = static_cast<double>(total_size);
  ...
  std::vector<clock::time_point> thread_start_times;
  ...
  for (ulint id = i; id < n; ++id) {
    ...
    threads.emplace_back(
      os_thread_create(buf_pool_create_thread_key,
      0,
      buf_pool_create,
      &buf_pool_ptr[id],
      size,
  ...
  for (ulint id = i; id < n; ++id) {
    ...
    int thread_minutes =
      static_cast<int>(thread_elapsed_seconds / 60);
    double thread_seconds_remaining =
      fmod(thread_elapsed_seconds, 60.0);

    ib::info(ER_IB_MSG_BUFFER_POOL_INSTANCE_INIT_COMPLETED,
      id, thread_minutes, thread_seconds_remaining);
  ...
  if (err != DB_SUCCESS) {
    ...
    int error_minutes =
      static_cast<int>(elapsed_seconds / 60);
    double error_seconds_remaining =
      fmod(elapsed_seconds, 60.0);
...
if (elapsed_seconds > 0.000001) {
  speed_gb_per_sec =
    (total_size_bytes / (1024.0 * 1024 * 1024)) / elapsed_seconds;
  }
...
/* abort buffer pool load */
  ...
  message_interval) {
    if (message_interval > std::chrono::minutes
      {
      15
      }
    ) {
...
// Take shard mutex to protect against concurrent resize erase.
  std::shared_lock<std::shared_mutex> lock(chunk_map_mutexes[shard_idx]);
...
/* Ensure latches are initialized before the block becomes accessible. */
