...
static my_hash_value_type calc_hash(const HASH *hash, const uchar *key,
                                    size_t length) {
  ulong nr1 = 1, nr2 = 4;
...
bool my_hash_init(HASH *hash, const CHARSET_INFO *charset, ulong reserve_size,
  ...
  retval = my_init_dynamic_array(&hash->array, psi_key, sizeof(HASH_LINK),
                                 NULL, /* init_buffer */
                                 reserve_size, 0);
  DBUG_RETURN(retval);
}
...
static inline void my_hash_free_elements(HASH *hash) {
  if (hash->free_element) {
    ...
    while (data < end) (*hash->free_element)((data++)->data);
  }
  hash->records = 0;
}
...
static uint my_hash_rec_mask(const HASH *hash, HASH_LINK *pos, size_t buffmax,
                             size_t maxlength) {
  size_t length;
...
my_hash_value_type my_calc_hash(const HASH *hash, const uchar *key,
                                size_t length) {
...
uchar *my_hash_first(const HASH *hash, const uchar *key, size_t length,
  ...
  if (my_hash_inited(hash))
    res = my_hash_first_from_hash_value(
        hash, calc_hash(hash, key, length ? length : hash->key_length), key,
        length, current_record);
  else
    res = 0;
  return res;
}
...
static int hashcmp(const HASH *hash, HASH_LINK *pos, const uchar *key,
  ...
  return (
      (length && length != rec_keylength) ||
      my_strnncoll(hash->charset, rec_key, rec_keylength, key, rec_keylength));
}
...
bool my_hash_insert(HASH *info, const uchar *record) {
  ...
  if (HASH_UNIQUE & info->flags) {
    ...
    if (my_hash_search(info, key, idx)) return (true); /* Duplicate entry */
  }
  ...
  idx =
      my_hash_mask(rec_hashnr(info, record), info->blength, info->records + 1);
  pos = data + idx;
  if (pos == empty) {
    ...
    if (pos == gpos) {
      ...
      pos->data = const_cast<uchar *>(record);
      pos->next = NO_RECORD;
      movelink(data, (uint)(pos - data), (uint)(gpos - data),
               (uint)(empty - data));
    }
  }
...
bool my_hash_delete(HASH *hash, uchar *record) {
  ...
  if (pos2 == my_hash_mask(pos_hashnr, blength,
                           hash->records + 1)) { /* Identical key-positions */
...
exit:
  ...
  if (hash->free_element) (*hash->free_element)(record);
  DBUG_RETURN(0);
}
...
bool my_hash_update(HASH *hash, uchar *record, uchar *old_key,
  ...
  if (HASH_UNIQUE & hash->flags) {
    ...
    if ((found = my_hash_first(hash, new_key, idx, &state))) {
      do {
        if (found != record) DBUG_RETURN(1); /* Duplicate entry */
  ...
  idx = my_hash_mask(
      calc_hash(hash, old_key,
                (old_key_length ? old_key_length : hash->key_length)),
      blength, records);
  new_index = my_hash_mask(rec_hashnr(hash, record), blength, records);
  if (idx == new_index) DBUG_RETURN(0); /* Nothing to do (No record check) */
  previous = 0;
  for (;;) {
    if ((pos = data + idx)->data == record) break;
    previous = pos;
    if ((idx = pos->next) == NO_RECORD) DBUG_RETURN(1); /* Not found in links */
  }
  org_link = *pos;
  empty = idx;
  ...
  if (new_index != new_pos_index) { /* Other record in wrong position */
    data[empty] = *pos;
...
bool my_hash_check(HASH *hash) {
  ...
  for (i = found = max_links = seek = 0; i < records; i++) {
    if (my_hash_rec_mask(hash, data + i, blength, records) == i) {
      ...
      seek++;
      links = 1;
      for (idx = data[i].next; idx != NO_RECORD && found < records + 1;
           idx = hash_info->next) {
        if (idx >= records) {
          DBUG_PRINT(
              "error",
        ...
        hash_info = data + idx;
        seek += ++links;
        if ((rec_link = my_hash_rec_mask(hash, hash_info, blength, records)) !=
            i) {
